...
function QuickActions() {
  return (
    <div className="flex flex-wrap gap-2 mb-4">
      <Link to="/import" className="btn-ghost text-xs">
        Import roster →
      </Link>
      <Link to="/recruits" className="btn-ghost text-xs">
        Track recruits →
      </Link>
      <Link to="/schedule" className="btn-ghost text-xs">
        Enter scores →
      </Link>
      <Link to="/roster" className="btn-ghost text-xs">
        View roster →
      </Link>
    </div>
  );
            ...
            <ul className="divide-y divide-border/60">
              {ratings.data?.overall.map((p) => (
                <li key={p.id} className="py-2 flex items-center gap-3">
                  <span
                    className={`stat-num text-base font-bold w-9 ${ratingColor(p.ovr)}`}
                  >
                    {p.ovr ?? '—'}
                  </span>
                  <Link
              ...
              <div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {POSITION_GROUPS.map((g) => (
                    <div
                      key={g}
                      className="rounded-md bg-bg-soft border border-border p-2 text-center"
                    >
                ...
                <div className="grid grid-cols-4 gap-2">
                  {(['Elite', 'Star', 'Impact', 'Normal'] as const).map((t) => (
                    <div
                      ...
                      className="rounded-md bg-bg-soft border border-border p-2 text-center"
                    >
                      <div className={`text-[10px] ${devTraitColor(t)}`}>
                        {t}
                      </div>
                      <div className="font-bold stat-num">
